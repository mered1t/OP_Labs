...
int main()
{
	...
	do {
		snailMovement = rand() % 2 + 1;
		 ...
		 if (snailPos <= 0) { std::cout << i << ") The snail is on the beginnig of the tree " << endl; i++; continue; }
		 if (snailPos >= lastEl) { std::cout << i << ") The snail is on the top of the tree " << endl; i++; continue; }	
	...
	else if(a[k] == 0){ std::cout << "The snail is on " << lastEl << " attitude"; }
